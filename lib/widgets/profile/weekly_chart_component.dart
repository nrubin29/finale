...
class _WeeklyChartComponentState extends State<WeeklyChartComponent>
  ...
  void initState() {
    ...
    _themeStreamSubscription = Preferences.themeColor.changes.listen(
      _updateSeries,
    );
  }
  ...
  void _initData() async {
    ...
    final (tracks, albums, artists, recentTracks) =
        await (
          Lastfm.getWeeklyTrackChart(
            widget.user,
            widget.chart,
          ...
          GetRecentTracksRequest(
            widget.user.name,
            from: widget.chart.fromDate,
            to: widget.chart.toDate,
          ).getAllData(),
        ).wait;

    final numScrobbles = tracks.fold(
      0,
